...
exports.syncSingleTenant = syncSingleTenant;
exports.syncAllTenants = syncAllTenants;
const admin = __importStar(require("firebase-admin"));
async function syncSingleTenant(tenantId) {
    var _a;
    ...
    const tenantName = tenantDoc.exists
        ? ((_a = tenantDoc.data()) === null || _a === void 0 ? void 0 : _a.name) || "未知社區"
        : "未知社區";
    ...
    console.log(`[${tenantId}] 找到 ${gatewayIds.length} 個啟用的通知點: ${gatewayIds.join(", ")}`);
    ...
    devicesSnapshot.docs.forEach((deviceDoc) => {
        ...
        // 檢查是否需要更新（避免不必要的寫入）
        const needsUpdate = gatewayIds.length !== currentIds.length ||
            !gatewayIds.every((id) => currentIds.includes(id));
        if (needsUpdate) {
            batch.update(deviceDoc.ref, {
                inheritedNotificationPointIds: gatewayIds.length > 0 ? gatewayIds : admin.firestore.FieldValue.delete(),
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            });
            updateCount++;
            console.log(`[${tenantId}] 更新設備 ${deviceDoc.id}: ${currentIds.length} → ${gatewayIds.length} 個通知點`);
        }
    });
    if (updateCount > 0) {
        await batch.commit();
...
async function syncAllTenants() {
    ...
    console.log("同步完成");
    console.log("========================================");
    const totalDevicesUpdated = results.reduce((sum, r) => sum + r.devicesUpdated, 0);
    ...
    results.forEach((result) => {
        if (result.devicesUpdated > 0 || result.notificationPoints > 0) {
            console.log(`  ${result.tenantName || result.tenantId}: ${result.devicesUpdated} 設備, ${result.notificationPoints} 通知點`);
        }
    });
    console.log("\n========================================\n");
    return results;
...
//# sourceMappingURL=syncTenantNotificationPoints.js.map