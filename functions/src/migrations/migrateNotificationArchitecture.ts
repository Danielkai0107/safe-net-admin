 ...
 * 
 * 遷移內容：
 * 1. 將 app_users.fcmToken 複製到對應設備的 devices.fcmToken
 * 2. 將 appUserNotificationPoints 複製到 devices/{deviceId}/notificationPoints 子集合
 * 3. 為有社區 tag 的設備填充 inheritedNotificationPointIds
 */

interface MigrationStats {
  fcmTokensMigrated: number;
  fcmTokensSkipped: number;
  notificationPointsMigrated: number;
  notificationPointsSkipped: number;
  inheritedPointsSet: number;
  errors: string[];
}

export async function migrateNotificationArchitecture(
  dryRun: boolean = true
  ...
  try {
    ...
    for (const tenantDoc of tenantsSnapshot.docs) {
      ...
      const gatewayIds = tenantNotificationPointsQuery.docs.map(
        (doc) => doc.data().gatewayId
      );
      ...
      for (const deviceDoc of devicesQuery.docs) {
        ...
        if (
          deviceData.inheritedNotificationPointIds &&
